...
async function getAffectedTables (pg, sql) {
    try {
        ...
        const rows = await execQuery({ pg, sql: query, params: {}, readOnly: true });
        ...
        throw new Error(`Could not fetch metadata about the affected tables: ${msg}`);
    }
}
...
async function getForeignTableUpdatedAt (pg, table) {
    try {
        ...
        const remoteNameResult = await execQuery({ pg, sql: remoteNameQuery, params: {}, readOnly: true });
        ...
        if (foreignQualifiedName === null) {
            throw new Error('Cannot compute qualified name of foreign table');
        }
        ...
        * and is on the same schema as the queried table. */
        ...
        const remoteUpdatedAtResult = await execQuery({ pg, sql: remoteUpdatedAtQuery, params: {}, readOnly: true });
        ...
        const msg = err.message ? err.message : err;
        throw new Error(`Could not fetch metadata for relation '${table.id_name}': ${msg}`);
    }
}
...
async function getViewUpdatedAt (pg, view) {
    ...
    const readOnly = true;
    const params = {};
    const viewDefinition = await execQuery({ pg, sql: materializedViewDefinitionQuery, params, readOnly });
...
async function getTablesMetadata (pg, tableArray) {
    try {
        ...
        * by their `::regclass::oid` */
        ...
        const metadata = await execQuery({ pg, sql: metadataQuery, params: {}, readOnly: true });
 ...
 */
module.exports.getQueryMetadataModel = async function getQueryMetadataModel (pg, sql, callback) {
