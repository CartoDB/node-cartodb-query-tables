...
function execQueryPromise ({ pg, sql, params, readOnly = true } = {}) {
...
async function getViewUpdatedAt (pg, view) {
    ...
    const readOnly = true;
    const params = {};
    const viewDefinition = await execQueryPromise({ pg, sql: materializedViewDefinitionQuery, params, readOnly });

    const query = viewDefinition[0].definition;
    const getAffectedTablesPromise = promisify(getAffectedTables);
    const affectedTableView = await getAffectedTablesPromise(pg, query);

    const getTablesMetadataPromise = promisify(getTablesMetadata);

    const res = await getTablesMetadataPromise(pg, affectedTableView);
    ...
    for (const r of res) {
        ...
        if (rDate.getTime() > latestDate.getTime()) {
            latest = r;
        }
    }
