...
/* Returns a promise with the serialization of a promise list */
...
function getTablesMetadata (pg, tableArray, callback) {
    ...
    pg.query(metadataQuery, {}, (err, result) => {
        ...
        const updatedAtPromiseCollection = rows
                .map(row => row.relkind === 'f' ?
                    getForeignTableUpdatedAtPromise(pg, row).then(updated_at => Object.assign({}, row, {updated_at})) :
                    Promise.resolve(row));
        serializePromises(updatedAtPromiseCollection)
...
module.exports.getQueryMetadataModel = function (pg, sql, callback) {
    ...
    const getAffectedTablesPromiseCollection = statements.map(query => getAffectedTablesPromise(pg, query));

    serializePromises(getAffectedTablesPromiseCollection)
        .then((values) => {
            const merged = flat(values);
            if (merged.length === 0) {
