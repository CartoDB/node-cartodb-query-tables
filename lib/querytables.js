...
function getForeignTableUpdatedAt (pg, table, callback) {
    const remoteNameQuery = `
        ...
        (
            SELECT ftoptions FROM pg_foreign_table WHERE ftrelid=${encodePGName(table.idname)}::regclass LIMIT 1
        ),
    ...
    pg.query(remoteNameQuery, {}, (err, result) => {
        if (err) {
            const msg = err.message ? err.message : err;
            return callback(new Error(`Could not fetch metadata for relation '${table.idname}': ${msg}`));
        }

        const rows = result.rows || [];
        const foreignQualifiedName = rows.length > 0 ? rows[0].cdb_fdw_qual_name : null;
        if (foreignQualifiedName === null) {
            return callback(new Error(`Could not fetch remote qualified name for relation '${table.idname}'`));
        }
        ...
        //TODO: Select * from local_schema

        pg.query(remoteUpdatedAtQuery, {}, (err, result) => {
            if (err) {
                const msg = err.message ? err.message : err;
                return callback(new Error(`Could not fetch update time for relation '${table.idname}': ${msg}`));
            }
...
function getTablesMetadata (pg, tableArray, callback) {
    ...
    metadataQuery += `
        ...
        SELECT * FROM cdb_table_metadata ORDER BY id_name;`;
    ...
    pg.query(metadataQuery, {}, (err, result) => {
        ...
        const promiseCollection = rows
                .filter(r => r.relking === 'f')
                .map(row => getForeignTableUpdatedAtPromise(pg, row)
                    .then(updated_at => Object.assign(row, {updated_at})));
...
module.exports.getQueryMetadataModel = function (pg, sql, callback) {
    ...
    const promiseCollection = statements.map(query => getAffectedTablesPromise(pg, query));
    ...
    Promise.all(promiseCollection)
        .then((values) => {
            ...
            getTablesMetadata(pg, merged, (err, metadata) => {
                ...
                return callback(null, new QueryMetadataModel(metadata || []));
            });
        })
        .catch(err => callback(err));
...
//
//
//Add schema retrieval / LIMIT 0 function.
//CDB_QueryTablesText to pure SQL..
//CDB_QueryTables_Updated_At to pure SQL.
//Add CDB_EstimateRowCount pure SQL function.
//Check if decimal.js is still necessary after the improvements in the aggregations or using native JS is enough.
//Maybe: Add _CDB_UserQuotaInBytes / CDB_UserDataSize equivalent functions so they can be used in the SQL API
// when the extension isn't available (I need to think what return value would make sense).