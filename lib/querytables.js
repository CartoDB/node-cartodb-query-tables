...
module.exports.getAffectedTableNamesFromQuery = function (pg, sql, callback) {
    const query = 'SELECT CDB_QueryTablesText($windshaft$' + sql_t0(sql) + '$windshaft$)';
...
module.exports.getAffectedTablesFromQuery = function (pg, sql, callback) {
    const query = 'SELECT * FROM CDB_QueryTables_Updated_At($windshaft$' + sql_t0(sql) + '$windshaft$)';
