...
function getTablesMetadata (pg, tableArray, callback) {
    let sql = `WITH cdb_table_oids AS (
        SELECT format('%s.%s', quote_ident('${tableArray[0].schema_name}'),
                               quote_ident('${tableArray[0].table_name}'))::regclass::oid as reloid
    `;

    for (let i = 1; i < tableArray.length; i++) {
        sql += `UNION ALL
        SELECT format('%s.%s', quote_ident('${tableArray[i].schema_name}'),
                               quote_ident('${tableArray[i].table_name}'))::regclass::oid as reloid
        `;
    }
    ...
    sql += `
        ...
        ORDER BY dbname, schema_name, table_name`;

    //  TODO: UpdatedAt
    pg.query(sql, null, (err, result) => {
        if (err) {
            const msg = err.message ? err.message : err;
            return callback(new Error('Could not fetch metadata about the affected tables: ' + msg));
        }
