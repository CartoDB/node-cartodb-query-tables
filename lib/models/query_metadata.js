...
class QueryMetadata {
    ...
    getCacheChannel (skipNotUpdatedAtTables) {
        const groupedTables = this.getTables(skipNotUpdatedAtTables).reduce((grouped, table) => {
            if (!Object.prototype.hasOwnProperty.call(grouped, table.dbname)) {
...
function filterTablesWithUpdatedAt (table) {
    return Object.prototype.hasOwnProperty.call(table, 'updated_at') && table.updated_at !== undefined && table.updated_at !== null;
}

const analysisTableRegex = /^analysis_[a-f0-9]{10}_[a-f0-9]{40}$/;

function filterAnalysisTables (table) {
    return Object.prototype.hasOwnProperty.call(table, 'table_name') && !analysisTableRegex.test(table.table_name);
}
